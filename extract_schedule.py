...
def setup_tesseract():
    ...
    # Force use of the 'tesseract' command on PATH (Linux/Codespaces)
    pytesseract.pytesseract.tesseract_cmd = 'tesseract'
    print(f"Using tesseract executable at: {pytesseract.pytesseract.tesseract_cmd}") cv2.imread(flat_path)
...
def normalize_and_save(table, output_csv):
    ...
    pd.DataFrame(rows, columns=header).to_csv(output_csv, index=False)
